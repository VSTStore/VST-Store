...
if(isset($_POST['form1'])) {
    ...
    for($i=1;$i<=count($arr1);$i++) {
        ...
        if($table_quantity[$temp_index] < $arr2[$i]) {
        	$allow_update = 0;
            $error_message .= '"'.$arr2[$i].'" товары недоступны для "'.$arr3[$i].'"\n';
        } else {
            $_SESSION['cart_p_qty'][$i] = $arr2[$i];
        }
    }
    $error_message .= '\nКоличество других товаров успешно обновлено!';
    ?>
    
    <?php if($allow_update == 0): ?>
    	<script>alert('<?php echo $error_message; ?>');</script>
	<?php else: ?>
		<script>alert('Обновление количества всех товаров выполнено успешно!');</script>
	<?php endif; ?>
    <?php
		...
		<div class="row">
			<div class="col-md-12">
                ...
                <?php if(!isset($_SESSION['cart_p_id'])): ?>
                    <?php echo '<h2 class="text-center">Корзина пустая</h2></br>'; ?>
                    <?php echo '<h4 class="text-center">Для начала добавьте плагины</h4>'; ?>
                <?php else: ?>
				...
				<div class="cart">
                    <table class="table table-responsive table-hover table-bordered">
                        ...
                        <?php endfor; ?>
                        <tr>
                            <th colspan="5" class="total-text">Итого</th>
